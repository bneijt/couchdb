...
-export([att_foldl/3,range_att_foldl/5,att_foldl_decode/3,get_validate_doc_fun/1,get_validate_doc_read_fun/1]).
...
get_validate_doc_read_fun(#doc{body={Props}}=DDoc) ->
    case couch_util:get_value(<<"validate_doc_read">>, Props) of
    undefined ->
        nil;
    _Else ->
        fun(ReadDoc, Ctx, SecObj) ->
            couch_query_servers:validate_doc_read(DDoc, ReadDoc, Ctx, SecObj)
        end
    end.
